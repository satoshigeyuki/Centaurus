...
class JsonListener(object):
    ...
    def parseFeatureList(self, ctx):
        return tuple(ctx)
    def parseFeatureObject(self, ctx):
        return ctx[0]
    def parseTargetFeatureObject(self, ctx):
        return ctx[2]
    def parseGeometryObject(self, ctx):
        return ctx[0]
    def parsePolygonGeometryDictionary(self, ctx):
        return ctx[0]
    def parseMultiPolygonGeometryDictionary(self, ctx):
        return ctx[0]
    def parsePolygonCoordinateObject(self, ctx):
        return tuple(ctx)
    def parseMultiPolygonCoordinateObject(self, ctx):
        return tuple(ctx)
    def parsePolygonCoordinateElement(self, ctx):
        return tuple(ctx)
    def parseMultiPolygonCoordinateElement(self, ctx):
        return tuple(ctx)
    def parseCoordinateList(self, ctx):
        return tuple(ctx)
    def parseCoordinate(self, ctx):
        return tuple(ctx)
...
if __name__ == '__main__':
    ...
    context = Context('../grammar/json3.cgr', num_workers)
